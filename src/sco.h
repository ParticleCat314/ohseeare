...
struct ScoImage {
  std::vector<uint8_t>
      rgba; // row-major, top-to-bottom, 4 bytes/pixel (R G B A)
  int width = 0;
  int height = 0;
};
...
static ScoImage grab_desktop_impl() {
  ...
  img.rgba.resize((size_t)img.width * img.height * 4);
  GetDIBits(memDC, hBmp, 0, img.height, img.rgba.data(), (BITMAPINFO *)&bi,
            DIB_RGB_COLORS);
...
ScoImage sco_grab_desktop() { return grab_desktop_impl(); }
...
static void push_quad(std::vector<OVtx> &verts, float x0, float y0, float x1,
                      float y1, float u0, float v0, float u1, float v1, float r,
...
static void push_line(std::vector<OVtx> &verts, float x0, float y0, float x1,
...
ScoImage sco_capture_region() {
  ...
  while (!glfwWindowShouldClose(overlay)) {
    printf(
        "\rSelection in progress... Press Escape or right-click to cancel. ");
    glfwPollEvents();
    ...
    bool curr_lmb =
        glfwGetMouseButton(overlay, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS;
    ...
    bool has_sel =
        (sel_max_x - sel_min_x) > 1.0f && (sel_max_y - sel_min_y) > 1.0f;
    ...
    verts.clear();
    push_quad(verts, 0, 0, disp_w, disp_h, 0, 0, 0, 0, 0, 0, 0,
              120.0f / 255.0f);
    glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr)(verts.size() * sizeof(OVtx)),
                 verts.data(), GL_STREAM_DRAW);
    ...
    if (has_sel) {
      ...
      verts.clear();
      push_quad(verts, sel_min_x, sel_min_y, sel_max_x, sel_max_y, u0, v0, u1,
                v1, 1, 1, 1, 1);
      glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr)(verts.size() * sizeof(OVtx)),
                   verts.data(), GL_STREAM_DRAW);
      glUniform1i(loc_mode, 1);
      ...
      verts.clear();
      push_line(verts, sel_min_x, sel_min_y, sel_max_x, sel_min_y, br, bg, bb,
                1);
      push_line(verts, sel_max_x, sel_min_y, sel_max_x, sel_max_y, br, bg, bb,
                1);
      push_line(verts, sel_max_x, sel_max_y, sel_min_x, sel_max_y, br, bg, bb,
                1);
      push_line(verts, sel_min_x, sel_max_y, sel_min_x, sel_min_y, br, bg, bb,
                1);
      glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr)(verts.size() * sizeof(OVtx)),
                   verts.data(), GL_STREAM_DRAW);
      glUniform1i(loc_mode, 0);
